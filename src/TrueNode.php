...
final class TrueNode
{
    ...
    /**
     * Osculating lunar node from the current MoshierMoon position/speed.
     *
     * @return array{0:float, 1:float, 2:float, 3:float, 4:float, 5:float}
     */
    public static function geocentric(float $tjdEt): array
    {
        $position = self::nodeCartesian($tjdEt);
        $previous = self::nodeCartesian($tjdEt - self::SPEED_INTERVAL);
        $next = self::nodeCartesian($tjdEt + self::SPEED_INTERVAL);
     ...
     */
    private static function nodeCartesian(float $tjdEt): array
    {
        ...
        return self::correctNodeDistance($node, $moon);
    }

    /**
     * @param array{0:float, 1:float, 2:float} $node
     * @param array{0:float, 1:float, 2:float, 3:float, 4:float, 5:float} $moon
     * @return array{0:float, 1:float, 2:float}
     */
    private static function correctNodeDistance(array $node, array $moon): array
    {
        ...
        $nodeCosE = cos(
            2.0 * atan(
                tan($nodeTrueAnomaly / 2.0)
                / sqrt((1.0 + $eccentricity) / (1.0 - $eccentricity))
            )
        );

        $nodeRadius = $semiAxis * (1.0 - $eccentricity * $nodeCosE);
        $oldRadius = sqrt(self::squareSum($node));
        ...
        return [
            $node[0] * $nodeRadius / $oldRadius,
            $node[1] * $nodeRadius / $oldRadius,
            $node[2] * $nodeRadius / $oldRadius,
        ];
    }
